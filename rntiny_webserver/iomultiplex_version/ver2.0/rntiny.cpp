...
#define WORKER_BALANCE_FACTOR ((double)7/10)
...
//			queue<int> ready_que_wd;
			...
			while(true)
			{
				...
				if(client_num == 0)
				{
					...
					sign = true;
					sign1 = true;
				//printf("worker process %d wait over, client num: %d\n", getpid(),client_num);
				}else if(listen_ptr->try_wait() == 0)
					sign = true;
				...
				if(sign)
				{
					if(listen_ptr->is_valid()&&listen_ptr->is_accept())
					{
						...
						}while(client_num < WORKER_BALANCE_FACTOR*((double)WORKER_LISTEN_NUM));
						if(sign1&&client_num >= WORKER_BALANCE_FACTOR*((double)WORKER_LISTEN_NUM))
						{
							printf("post_src, client_num %d\n", client_num);
							listen_ptr->post_src();
						}
					}
					fflush(log_file_ptr);
	...
	while(true)
	{
		...
		for(int cnt = 0; cnt < read_fd_num; ++cnt)
		{
			...
			listen_ptr->post(); //v()
			listen_ptr->post_src(); //add source 
			//printf("hello here is master\n");
		}
	//	printf("master process wait over\n");
	}
	while(true)
	{
		if(waitpid(-1,0,0) < 0)
