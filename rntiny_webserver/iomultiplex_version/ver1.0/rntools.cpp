...
ssize_t rnio_writen(int fd, void *usrbuf, size_t n)
{
	...
	ssize_t nwritten = 0;
//	unsigned char *ptr = reinterpret_cast<unsigned char*>(usrbuf);
	unsigned char *ptr = (unsigned char*)(usrbuf);
	while(nleft>0)
	{
		if((nwritten = write(fd, ptr, nleft))<0)
		{
			if(errno == EINTR||errno == EAGAIN) //consider nonblocking
				nwritten = 0;
			else
				return -1;
		}
...
//this function only follow rnio_readlineb()
void rnio_retrieve(rnio_t *ptr, int cnt)
{
	if(cnt > RNMAXLINE - ptr->rnio_cnt)
		cnt = RNMAXLINE - ptr->rnio_cnt;
	ptr->rnio_bufptr -= cnt;
	ptr->rnio_cnt += cnt;
}
static ssize_t rnio_read(rnio_t *rnptr, unsigned char *usrbuf, size_t n) //linux read version with buf
{
	...
	{
		memset(rnptr->rnio_buf, 0, sizeof(rnptr->rnio_buf));
		rnptr->rnio_cnt = read(rnptr->rnio_fd, rnptr->rnio_buf, sizeof(rnptr->rnio_buf));
#ifndef DEBUG
			printf("============read() errno:%d ret:%d buf:%s, bufsize:%d\n",errno,rnptr->rnio_cnt, rnptr->rnio_buf, strlen(rnptr->rnio_buf));
			perror("=====");
#endif
...
//	unsigned char c, *rnbuf = reinterpret_cast<unsigned char*>(usrbuf);
	unsigned char c, *rnbuf = (unsigned char*)(usrbuf);
	int res;
	size_t n;
	for(n = 1; n < maxlen; ++n)
	{
